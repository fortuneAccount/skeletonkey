...
if ((tstidir = "assets")or(tstidir = "bin")or(tstidir = "binaries"))
	{
		home= %tstipth%
	}
source= %home%\assets
		...
		ifexist,%rjemut%\
			{
				RJSYSTEMS= %rjsyst%
				RJEMUF= %rjemut%
				IniWrite, "%RJEMUF%",%home%\Settings.ini,GLOBAL,emulators_directory
				IniWrite, "%RJSYSTEMS%",%home%\Settings.ini,GLOBAL,systems_directory
...
if (ADJDIRS = 0)
	{
		...
		iniwrite,"0",%home%\Settings.ini,GLOBAL,global_filter
		return
	}
iniwrite,"1",%home%\Settings.ini,GLOBAL,global_filter
...
IniWrite, "%RJEMUF%",%home%\Settings.ini,GLOBAL,emulators_directory
IniWrite, "%LNKVRJ%",%home%\Settings.ini,GLOBAL,systems_directory
exlst= |
exls= |
