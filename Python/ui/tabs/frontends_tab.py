"""
ui/tabs/frontends_tab.py

Frontends tab – download, configure and launch gaming frontends
(EmulationStation, RetroFE, Pegasus, etc.).
"""
...
class FrontendsTab(BaseTab):
    ...
    def _build_ui(self):
        ...
        left = QWidget()
        ll = QVBoxLayout(left)
        ll.addWidget(QLabel("Frontends"))
        self._fe_list = QListWidget()
        self._fe_list.currentTextChanged.connect(self._on_fe_selected)
        ll.addWidget(self._fe_list)
        splitter.addWidget(left)

        right = QWidget()
        rl = QVBoxLayout(right)

        info = QGroupBox("Frontend Details")
        ...
        self._launch_btn = QPushButton("Launch Frontend")
    ...
    def _populate(self):
        self._fe_list.clear()
        for entry in self._emus.frontends():
            self._fe_list.addItem(entry.name)

    def _on_fe_selected(self, name: str):
        entry = self._emus.get(name)
        if not entry:
            return
        self._name_label.setText(entry.name)
        self._exe_label.setText(entry.exe)
        self._install_path.setText(str(app_home() / "Frontends" / entry.name))

    def _download(self):
        name = self._fe_list.currentItem()
        if not name:
            return
    ...
    def _on_download_finished(self, success: bool):
        ...
        self.set_status(
            "Frontend download complete." if success else "Download failed.")
